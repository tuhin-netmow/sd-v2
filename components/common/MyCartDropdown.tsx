...
const MyCartDropdown = () => {
  ...
  const removeItem = (ele: any) => {
    ...
    var cartItemTotalElement = document.getElementById("cart-item-total");

    if (cartItemTotalElement) {
      var subTotal = parseInt(cartItemTotalElement.innerHTML);
      cartItemTotalElement.innerHTML = (subTotal - price).toFixed(2);
    } else {
      console.error("Element with ID 'cart-item-total' not found");
    }
